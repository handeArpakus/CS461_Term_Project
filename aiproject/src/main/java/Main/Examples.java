...
public class Examples {
    ...
    public Examples(Dictionary dictionary) throws JWNLException {
        ...
        ACCOMPLISH = dictionary.getIndexWord(POS.NOUN, "white");
    ...
    private void demonstrateListOperation(IndexWord word)  {
        // Get all of the hypernyms (parents) of the first sense of <var>word</var>
        PointerTargetNodeList hypernyms = null;
        try {
            hypernyms = PointerUtils.getDirectHyponyms(word.getSenses().get(0));
    ...
    private void demonstrateAsymmetricRelationshipOperation(IndexWord start, IndexWord end) throws JWNLException, CloneNotSupportedException {
        ...
        System.out.println("Common Parent Index: " + ((AsymmetricRelationship) list.get(1)).getCommonParentIndex());
        System.out.println("Depth: " + list.get(1).getDepth());
    }
